...
#include "game/kernel/jak1/sm64_audio_thread.h"
#include "game/kernel/jak1/kmachine.h"
#include "game/sce/libscf.h"

#include <SDL3/SDL.h> // Corrected include path for SDL3

static SDL_AudioStream* sm64_audio_stream = nullptr;
static SDL_AudioDeviceID sm64_audio_device = 0;

void InitMarioAudio() {
    printf("[libsm64][DEBUG] Entering InitMarioAudio()\n");

    // --- CORRECTED ADDED CODE: Enumerate Audio Devices for Playback ---
    int num_playback_devices = 0;
    // Get a list of all available audio playback devices
    SDL_AudioDeviceID *playback_devices = SDL_GetAudioPlaybackDevices(&num_playback_devices);

    if (!playback_devices) {
        // If SDL_GetAudioPlaybackDevices fails
        fprintf(stderr, "[libsm64][ERROR] SDL_GetAudioPlaybackDevices failed: %s\n", SDL_GetError());
    } else {
        printf("[libsm64][DEBUG] Found %d audio playback devices:\n", num_playback_devices);
        for (int i = 0; i < num_playback_devices; ++i) {
            SDL_AudioDeviceID device_id = playback_devices[i];
            const char* device_name = SDL_GetAudioDeviceName(device_id); // This function name remains the same
            printf("[libsm64][DEBUG]   Device %d: ID = %u, Name = \"%s\"\n", i, (unsigned int)device_id, device_name ? device_name : "UNKNOWN");
        }
        // Free the memory allocated by SDL_GetAudioPlaybackDevices
        SDL_free(playback_devices);
    }
    // --- END CORRECTED ADDED CODE ---
    ...
    printf("[libsm64][DEBUG] Attempting to open SDL audio device...\n");
    // Use SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK for the default device.
    // This is often 0, but using the macro is more explicit and future-proof.
    //
    sm64_audio_device = SDL_OpenAudioDevice(19, &desired); // Use 19 for FxSound Speakerssm64_audio_device = SDL_OpenAudioDevice(SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK, &desired);
    if (!sm64_audio_device) {
        printf("[libsm64][ERROR] Failed to open SDL audio device: %s\n", SDL_GetError());
        return;
    }
    printf("[libsm64][DEBUG] Successfully opened SDL audio device (ID: %u)\n", sm64_audio_device);
    ...
    SDL_AudioSpec dst_spec = src_spec; // No conversion needed, same format

    printf("[libsm64][DEBUG] Attempting to create SDL audio stream...\n");
    sm64_audio_stream = SDL_CreateAudioStream(&src_spec, &dst_spec);

    if (!sm64_audio_stream) {
        printf("[libsm64][ERROR] Failed to create SDL audio stream: %s\n", SDL_GetError());
        SDL_CloseAudioDevice(sm64_audio_device); // Close the device if stream creation fails
        sm64_audio_device = 0;
        return;
    }
    printf("[libsm64][DEBUG] Successfully created SDL audio stream.\n");

    printf("[libsm64][DEBUG] Attempting to resume SDL audio device...\n");
    if (SDL_ResumeAudioDevice(sm64_audio_device) < 0) { // SDL_ResumeAudioDevice now returns int, where 0 is success
        printf("[libsm64][ERROR] Failed to resume SDL audio device: %s\n", SDL_GetError());
        // Clean up stream and device on failure
        SDL_DestroyAudioStream(sm64_audio_stream);
        sm64_audio_stream = nullptr;
        SDL_CloseAudioDevice(sm64_audio_device);
        sm64_audio_device = 0;
        return;
    }
    printf("[libsm64][DEBUG] Successfully resumed SDL audio device.\n");
    printf("[libsm64][DEBUG] Exiting InitMarioAudio() successfully.\n");
}

// Call this every frame or tick
void TickMarioAudio() {
    if (!sm64_audio_stream) {
        printf("[libsm64][WARN] TickMarioAudio called but audio stream is not initialized.\n");
        return;
    }
    ...
    sm64_audio_tick(0, samples, buffer);

    if (SDL_PutAudioStreamData(sm64_audio_stream, buffer, sizeof(buffer)) < 0) {
        printf("[libsm64][ERROR] Failed to put audio stream data: %s\n", SDL_GetError());
    }

    SDL_FlushAudioStream(sm64_audio_stream);
}


// --- NEW REVISED PlayTestWav function for SDL3 using SDL_OpenAudioDeviceStream ---
void PlayTestWav() {
...
s32 goal_main(int argc, const char* const* argv) {
    ...
    printf("[libsm64][DEBUG] Attempting SDL_Init(SDL_INIT_AUDIO)...\n");
    if (SDL_Init(SDL_INIT_AUDIO) < 0) {
        fprintf(stderr, "[libsm64][FATAL] SDL_Init(SDL_INIT_AUDIO) failed: %s\n", SDL_GetError());
        return 1; // Return an error code
    }
    printf("[libsm64][DEBUG] SDL_Init(SDL_INIT_AUDIO) successful. Initialized subsystems: %u\n", SDL_WasInit(0));

    // Check if other subsystems are already initialized or if there's a conflict
    Uint32 initialized_subsystems = SDL_WasInit(0);
    if (!(initialized_subsystems & SDL_INIT_AUDIO)) {
        fprintf(stderr, "[libsm64][FATAL] SDL_Init reported success, but audio subsystem not found in initialized flags!\n");
        SDL_Quit(); // Ensure cleanup
        return 1;
    }
    ...
    printf("[libsm64][DEBUG] Calling SDL_Quit()...\n");
    SDL_Quit(); // Clean up SDL subsystems
    printf("[libsm64][DEBUG] SDL_Quit() finished.\n");
    return result;
}
...
bool point_in_triangle_2d(float px, float pz, const int32_t v[3][3]) {
    ...
    float t = ((z2 - z1) * dX + (x1 - x2) * dZ) / D; // Corrected
...
void KernelCheckAndDispatch() {
    ...
    // Debug print for file opening
    if (!file.is_open()) {
        fprintf(stderr, "[libsm64][FATAL] Failed to open sm64.us.z64! Please ensure it's in the same directory as the executable.\n");
        // It might be appropriate to exit here or handle more gracefully
        // For now, let's allow it to continue to see subsequent errors if any.
    } else {
        printf("[libsm64][DEBUG] Successfully opened sm64.us.z64.\n");
    }
    ...
    if (romFileLength == (size_t)-1) { // Check for error on tellg()
        fprintf(stderr, "[libsm64][FATAL] Failed to get ROM file length.\n");
        return; // Exit or handle error
    }
    printf("[libsm64][DEBUG] ROM file length: %zu bytes.\n", romFileLength);

    romBuffer = new uint8_t[romFileLength + 1];
    file.seekg(0);
    file.read((char*)romBuffer, romFileLength);
    romBuffer[romFileLength] = 0;
    printf("[libsm64][DEBUG] ROM loaded into memory.\n");

    // Mario texture is 704x64 RGBA
    marioTexture = new uint8_t[4 * SM64_TEXTURE_WIDTH * SM64_TEXTURE_HEIGHT];
    printf("[libsm64][DEBUG] Mario texture buffer allocated.\n");

    // load libsm64
    printf("[libsm64][DEBUG] Calling sm64_global_init()...\n");
    sm64_global_init(romBuffer, marioTexture);
    printf("[libsm64][DEBUG] sm64_global_init() finished.\n");

    printf("[libsm64][DEBUG] Calling sm64_audio_init()...\n");
    sm64_audio_init(romBuffer);
    printf("[libsm64][DEBUG] sm64_audio_init() finished.\n");

    printf("[libsm64][DEBUG] Calling InitMarioAudio()...\n");
    //InitMarioAudio(); // Call the InitMarioAudio function here
    //PlayTestWav();


    printf("[libsm64][DEBUG] InitMarioAudio() call returned.\n");

    sm64_set_sound_volume(1.0f);
    printf("[libsm64][DEBUG] Sound volume set.\n");
    delete[] romBuffer;
    printf("[libsm64][DEBUG] ROM buffer deleted.\n");
